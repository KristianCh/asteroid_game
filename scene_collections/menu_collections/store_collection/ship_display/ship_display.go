components {
  id: "ship_display"
  component: "/scene_collections/menu_collections/store_collection/ship_display/ship_display.script"
  position {
    x: 0.0
    y: 0.0
    z: 0.0
  }
  rotation {
    x: 0.0
    y: 0.0
    z: 0.0
    w: 1.0
  }
}
components {
  id: "gui"
  component: "/scene_collections/menu_collections/store_collection/ship_display/ship_display.gui"
  position {
    x: 0.0
    y: 0.0
    z: 0.0
  }
  rotation {
    x: 0.0
    y: 0.0
    z: 0.0
    w: 1.0
  }
}
components {
  id: "battleship"
  component: "/assets/models/ships/battleship.model"
  position {
    x: 0.0
    y: 0.0
    z: 0.0
  }
  rotation {
    x: 0.81915206
    y: 0.0
    z: 0.0
    w: 0.57357645
  }
}
components {
  id: "laser_cutter"
  component: "/assets/models/ships/laser_cutter.model"
  position {
    x: 0.0
    y: 0.0
    z: 0.0
  }
  rotation {
    x: 0.81915206
    y: 0.0
    z: 0.0
    w: 0.57357645
  }
}
components {
  id: "minelayer"
  component: "/assets/models/ships/minelayer.model"
  position {
    x: 0.0
    y: 0.0
    z: 0.0
  }
  rotation {
    x: 0.81915206
    y: 0.0
    z: 0.0
    w: 0.57357645
  }
}
components {
  id: "missile_cruiser"
  component: "/assets/models/ships/missile_cruiser.model"
  position {
    x: 0.0
    y: 0.0
    z: 0.0
  }
  rotation {
    x: 0.81915206
    y: 0.0
    z: 0.0
    w: 0.57357645
  }
}
components {
  id: "mine_healer"
  component: "/assets/models/ships/mine_healer.model"
  position {
    x: 0.0
    y: 0.0
    z: 0.0
  }
  rotation {
    x: 0.81915206
    y: 0.0
    z: 0.0
    w: 0.57357645
  }
}
components {
  id: "gravity_tether"
  component: "/assets/models/ships/gravity_tether.model"
  position {
    x: 0.0
    y: 0.0
    z: 0.0
  }
  rotation {
    x: 0.81915206
    y: 0.0
    z: 0.0
    w: 0.57357645
  }
}
components {
  id: "graviton_trapper"
  component: "/assets/models/ships/graviton_trapper.model"
  position {
    x: 0.0
    y: 0.0
    z: 0.0
  }
  rotation {
    x: 0.81915206
    y: 0.0
    z: 0.0
    w: 0.57357645
  }
}
components {
  id: "energy_nuker"
  component: "/assets/models/ships/energy_nuker.model"
  position {
    x: 0.0
    y: 0.0
    z: 0.0
  }
  rotation {
    x: 0.81915206
    y: 0.0
    z: 0.0
    w: 0.57357645
  }
}
components {
  id: "ram"
  component: "/assets/models/ships/ram.model"
  position {
    x: 0.0
    y: 0.0
    z: 0.0
  }
  rotation {
    x: 0.81915206
    y: 0.0
    z: 0.0
    w: 0.57357645
  }
}
components {
  id: "fleet_carrier"
  component: "/assets/models/ships/fleet_carrier.model"
  position {
    x: 0.0
    y: 0.0
    z: 0.0
  }
  rotation {
    x: 0.81915206
    y: 0.0
    z: 0.0
    w: 0.57357645
  }
}
components {
  id: "kamikaze_carrier"
  component: "/assets/models/ships/kamikaze_carrier.model"
  position {
    x: 0.0
    y: 0.0
    z: 0.0
  }
  rotation {
    x: 0.81915206
    y: 0.0
    z: 0.0
    w: 0.57357645
  }
}
